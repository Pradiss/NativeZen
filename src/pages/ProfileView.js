...
export default function ProfileView({ navigation }) {
  ...
  useEffect(() => {
    ...
    const LoadingUsers = async () => {
      try {
        const res = await axios.get(
          `https://erick5457.c44.integrator.host/api/usuarios/${idUsuario}`,
          authHeader
        );
        setUsers(res.data);
      } catch (error) {
    ...
    const LoadingCategory = async () => {
      try {
        const res = await axios.get(
          "https://erick5457.c44.integrator.host/api/categorias"
        );
    ...
    >
      <View style={{ paddingTop: 56 }}>
        ...
        <View style={{ alignItems: "start" }}>
          <Text style={{ fontSize: 38, fontWeight: 600, marginTop: 16 }}>
